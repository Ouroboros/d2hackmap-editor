...
const helpEntries = computed<HelpEntry[]>(() => [
  {
    id: 'common',
    label: '通用',
    intro: '先看这一页，再去看具体功能页。你可以把编辑器理解成“按原 CFG 引用链加载外部配置，再通过编辑器入口接入当前预设和用户配置”。',
    blocks: [
      {
        title: '先理解这 6 个概念',
        lines: [
          '1. 入口文件：d2hackmap.gen.cfg 只负责导入 d2hackmap.editor.profile.cfg 和 d2hackmap.editor.user.cfg，不存放具体用户规则。',
          '2. 预设项：来自 d2hackmap.editor.profile.cfg，是当前预设的基础规则，可以编辑、注释、删除、拖拽排序。',
          '3. 用户配置项：来自 d2hackmap.editor.user.cfg，是你在当前预设基础上的个人微调，可以编辑、注释、删除、拖拽排序。',
          '4. 外部项：来自 d2hackmap.default.cfg、d2hackmap.cfg 或其他引用文件。外部项是只读的，不能直接改。',
          '5. 已注释项：保存后仍保留在所属 cfg 里，但这一条不会生效，适合“先关掉，稍后再开”。',
          '6. 已删除项：保存后会从所属可编辑配置中移除，适合你确认不再需要时使用。'
        ],
        exampleTitle: '例子',
        exampleLines: [
          '你看到一条灰色的外部项，想把它改成自己的规则：先点右侧“+”，复制到用户配置，再修改复制出来的那一条。',
      ...
      },
      {
        ...
        steps: [
          ...
          '确认编辑器已经把 d2hackmap.gen.cfg 接入原 CFG 引用链；它只是入口文件，不是用户配置。',
          '先在各页面浏览外部项和预设项，决定哪些需要复制到用户配置。',
          '临时微调用“保存用户配置”，要改当前阶段基础规则才用“保存当前预设”。如果只是切页面或关闭程序，改动不会自动写回 cfg。'
        ],
        exampleTitle: '例子',
        exampleLines: [
          '第一次使用时，可以先进入“自动拾取”页，找到灰色外部规则，复制几条你想保留的规则到用户配置，再慢慢改。'
        ]
      },
      {
        title: '顺序与生效逻辑',
        lines: [
          ...
          '编辑器实际接入的是三层：外部配置先提供基础规则，d2hackmap.editor.profile.cfg 提供当前预设，d2hackmap.editor.user.cfg 提供你的个人微调。',
          'd2hackmap.gen.cfg 只是把当前预设和用户配置导入到 CFG 链里；具体规则分别保存在预设文件和用户配置文件中。',
          '灰色外部项本身不会被你直接改写；当你把它复制到用户配置后，真正用于覆盖的是复制出来的用户配置项。',
          '如果同一个目标被多条用户配置或预设规则同时命中，通常应该按列表顺序理解和整理，避免自己写出互相打架的规则。',
          '拖拽改变的不只是界面顺序，保存后对应 CFG 文件里的顺序也会跟着改变，所以拖拽本身就是在改生效顺序。'
        ],
        exampleTitle: '例子',
        exampleLines: [
          '例 1：自动拾取里前面一条写“所有珠宝都捡”，后面一条写“只捡高生命珠宝”。这种写法通常容易冲突，新手应尽量避免让宽泛规则挡在精确规则前面。',
          '例 2：物品颜色里前面一条把某类物品设成隐藏，后面又有一条更具体的用户配置规则想把其中一部分高亮显示。你需要明确这两条的先后关系，否则结果会和你想的不一样。',
          '例 3：灰色外部项已经定义了一条规则，你点“+”复制到用户配置并修改后，后续应该以你复制出来的用户配置项为准，而不是再去看灰色那条。'
        ],
        mistakesTitle: '常见错误',
        mistakes: [
          '只看单条规则内容，不看它在列表里的位置。',
          '写了两条都能命中同一目标的用户配置规则，却没有整理先后顺序。',
          '已经复制到用户配置了，却还以为灰色外部项才是最终生效的那条。',
  ...
  },
  {
    id: 'profiles',
    label: '预设',
    intro: '预设适合给不同阶段准备一套基础配置，比如开荒、中期、后期。它是编辑器自己的配置层，不会修改 d2hackmap.default.cfg。',
    blocks: [
      {
        title: '预设是什么',
        lines: [
          '预设可以理解成“当前阶段的基础方案”。例如你可以有一个“开荒”预设，里面保留更多药水、低级材料和过渡装备；也可以有一个“后期”预设，只保留高价值物品和成熟配方。',
          '顶部蓝色按钮会显示当前预设名称。点击它可以切换到其他已经保存过的预设，也可以在下拉列表里删除不需要的预设。',
          '预设文件由编辑器管理，属于 HackMap 目录里的编辑器辅助配置；它不是 d2hackmap.default.cfg，也不会直接改默认配置。'
        ],
        exampleTitle: '例子',
        exampleLines: [
          '你正在开荒，就切到“开荒”预设：自动拾取可以宽松一些，颜色提示也可以多一些。',
          '角色成型后，切到“后期”预设：只保留高价值物品、常用自动合成和更严格的属性限制。'
        ]
      },
      {
        title: '预设和用户配置的区别',
        lines: [
          '预设项显示为“预设: 名称”，背景是淡蓝色；用户配置项显示为“用户配置”。',
          '预设是当前阶段的基础配置，适合保存一批长期共用的规则。',
          '用户配置是你在当前预设基础上的个人微调，适合临时改动或少量覆盖。',
          '外部项和预设项都可以点“+”复制到用户配置。复制后你改的是用户配置，不会直接改原来的外部项或预设项。'
        ],
        exampleTitle: '例子',
        exampleLines: [
          '“开荒阶段多捡符文和宝石”适合放进开荒预设。',
          '“今天临时不捡某个材料”适合放进用户配置，之后可以恢复或删除。'
        ]
      },
      {
        title: '三个保存按钮',
        lines: [
          '保存用户配置：只保存用户配置层。适合保存你对当前预设做的临时微调。',
          '保存当前预设：只保存当前预设层，并把它同步到预设库。适合你明确想修改这个预设本身。',
          '保存到新预设：把当前预设和用户配置合并成一个新的预设，保存后切换到新预设，并清空当前用户配置层。',
          '如果你只是想保存日常微调，优先点“保存用户配置”。不要把临时改动随手保存进预设。'
        ],
        exampleTitle: '例子',
        exampleLines: [
          '你从“开荒”预设开始，慢慢把拾取规则改成适合中期使用。确认这套规则以后，点“保存到新预设”，命名为“中期”。',
          '你只是临时想多捡几种材料，点“保存用户配置”即可，不需要改“开荒”预设。'
        ]
      },
      {
        title: '切换和删除预设',
        lines: [
          '切换预设会替换当前活动预设。切换前如果有未保存改动，编辑器会提示确认。',
          '删除预设只删除预设库里的那个保存项。当前正在使用的活动预设如果来自被删除项，会回到未命名预设状态。',
          '删除预设不会修改 d2hackmap.default.cfg，也不会删除外部引用文件。'
        ],
        mistakesTitle: '常见错误',
        mistakes: [
          '切换预设前没有保存，导致刚改的用户配置或预设改动丢失。',
          '把“保存当前预设”和“保存用户配置”混用，导致临时微调被写进长期预设。',
          '以为删除预设会影响默认配置。不会，预设是编辑器自己的配置层。'
        ]
      }
    ]
  },
    ...
    blocks: [
      ...
      },
      {
        ...
        mistakes: [
          ...
          '只改了外部项但没有复制到用户配置，看起来改了，其实保存时不会写进去。'
        ]
      }
    ]
  },
    ...
    blocks: [
      ...
      },
      {
        ...
        mistakes: [
          ...
          '同时建了两条能匹配同一物品的用户配置项，导致后面的规则把前面的效果覆盖了。'
        ]
      }
    ]
  },
    ...
    blocks: [
      {
        ...
        exampleLines: [
          ...
          '如果你已经有一条外部按键绑定，最安全的做法仍然是先复制到用户配置，再改成自己的版本。'
        ]
      },
      {
