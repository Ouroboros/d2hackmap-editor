...
const helpEntries = computed<HelpEntry[]>(() => [
  {
    ...
    blocks: [
      ...
      },
      {
        title: '推荐操作顺序',
        steps: [
          '1. 先点“打开目录”，选择 HackMap 配置目录。',
          '2. 找到底部“输出到”显示的主配置文件，确认你改的是它。',
          '3. 先在各页面浏览外部项，决定哪些需要复制到主配置。',
          '4. 改完以后点“保存文件”。如果只是切页面或关闭程序，改动不会自动写回 cfg。'
        ],
        exampleTitle: '例子',
        exampleLines: [
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        title: '推荐做法',
        steps: [
          '1. 先决定你要改的是“普通物品颜色、符文颜色、还是金币颜色”。',
          '2. 在列表里找到接近的外部规则，优先复制再改。',
          '3. 如果没有现成规则，再点“添加”新建。',
          '4. 改完以后，可以用顶部颜色筛选检查自己到底改了哪些颜色。'
        ],
        mistakesTitle: '常见错误',
        mistakes: [
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        title: '推荐新手流程',
        steps: [
          '1. 先用外部已有规则作为模板，不要一开始就从空白写。',
          '2. 优先把“物品范围”配对，再决定拾取方式。',
          '3. 只有在“按物品类型还不够精确”时，再去配属性组。',
          '4. 每做完一类物品，就保存一次，避免一下改太多找不到问题。'
        ],
        mistakesTitle: '常见错误',
        mistakes: [
  ...
  },
  {
    ...
    blocks: [
      ...
      },
      {
        title: '选择器怎么用',
        steps: [
          '1. 点“当前公式列表”按钮，打开物品描述选择器。',
          '2. 左边是可选物品描述，点“+”加入当前公式。',
          '3. 右边是当前公式的配方物品列表，点“×”移除，点上下箭头调整顺序。',
          '4. 确认无误后点“确定”。'
        ],
        mistakesTitle: '常见错误',
        mistakes: [
