...
const helpEntries = computed<HelpEntry[]>(() => [
  {
    id: 'common',
    label: '通用',
    intro: '先看这一页，再去看具体功能页。你可以把编辑器理解成“把多个 cfg 文件读进来，最后只保存你自己的用户配置”。',
    blocks: [
      {
        title: '先理解这 4 个概念',
        lines: [
          '1. 用户配置项：你自己生成到 d2hackmap.gen.cfg 里的项目，可以直接改、注释、删除、拖拽排序。',
          '2. 外部项：来自 d2hackmap.default.cfg、d2hackmap.cfg 或其他引用文件。外部项是只读的，不能直接改。',
          '3. 已注释项：保存后仍保留在 cfg 里，但这一条不会生效，适合“先关掉，稍后再开”。',
          '4. 已删除项：保存后会从用户配置中移除，适合你确认不再需要时使用。'
        ],
        exampleTitle: '例子',
        exampleLines: [
      ...
      },
      {
        ...
        steps: [
          ...
          '找到底部“输出到”显示的用户配置文件，确认你改的是它。',
          '先在各页面浏览外部项，决定哪些需要复制到用户配置。',
      ...
      },
      {
        ...
        lines: [
          ...
          '你可以先把编辑器理解成两层：外部配置先提供“基础规则”，用户配置再在此基础上做覆盖和补充。',
          '灰色外部项本身不会被你直接改写；当你把它复制到用户配置后，真正生效的是你自己的那一条用户配置项。',
          '如果同一个目标被多条用户配置规则同时命中，通常应该按列表顺序理解和整理，避免自己写出互相打架的规则。',
